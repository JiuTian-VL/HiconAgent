            ...
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if self.config.trainer.record_logs:
                            os.makedirs(self.config.trainer.record_logs, exist_ok=True)
                            reward_tensor, reward_metrics, to_log = self.reward_fn(batch)
                            with open(self.config.trainer.record_logs + '/output.jsonl', 'a', encoding='utf-8') as f:
                                json.dump(to_log, f, ensure_ascii=False, separators=(",", ":"))
                                f.write("\n")
                        else:
                    ...
                    with _timer("old", timing_raw):
                        # print(self.actor_rollout_wg.do_compression)
                        # print(type(self.actor_rollout_wg))
                        # print(self.actor_rollout_wg)
                        # print(dir(self.actor_rollout_wg))

                        ## XYQConditionVL use do_compression, baseline set to None
                        if self.config.algorithm.use_compression:
                            batch.meta_info['do_compression'] = 1
                        else:
                            batch.meta_info['do_compression'] = None
                    ...
                    if self.use_reference_policy:
                        with _timer("ref", timing_raw):
                            ...
                            ## switch to pi_theta_wo
                            # batch.meta_info['do_compression'] = 0
                            # ref_log_probs = self.actor_rollout_wg.compute_log_probs(batch)
                            # batch = batch.union(ref_log_probs)
