...
def compute_policy_loss_ratio(
    old_log_probs: torch.Tensor,        # π_old (通常是 student 的旧策略)
    student_log_probs: torch.Tensor,    # log π_student(a_t | x_c)
    teacher_log_probs: torch.Tensor,    # log π_teacher(a_t | x_u)
    advantages: torch.Tensor,
    eos_mask: torch.Tensor,
    cliprange: float,
    c: float = None,                    # <-- 新增：IS 权重上截断（如 5 or 10）；None/<=0 则不启用
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """PPO 风格的策略损失 + student/teacher 比值加权（仅上截断 c，stop-grad）。"""

    # ===== 维持你原本的 PPO ratio & clip =====
    ...
    clipped_ratio = torch.exp(
        torch.clamp(
            negative_approx_kl,
            np.log(1.0 - cliprange),
            np.log(1.0 + cliprange),
        )
    )
    ppo_kl = VF.masked_mean(-negative_approx_kl, eos_mask)

    # 原始 PPO 的逐 token 损失（还没做 masked_mean）
    pg_losses  = -advantages * ratio
    pg_losses2 = -advantages * clipped_ratio
    base_pg    = torch.max(pg_losses, pg_losses2)   # 经典 PPO 的 min/max 技巧

    # ===== 新增：student/teacher 的比值权重（仅上截断 c，且 stop-grad）=====
    # r = exp(log π_s - log π_t)；这是“student/teacher”的标准 IS 方向
    if c is not None and c > 0:
        with torch.no_grad():
            tis_weight = torch.exp(teacher_log_probs - student_log_probs)
            tis_weight = torch.clamp(tis_weight, max=c)   # 仅上截断控方差
        # 乘到 PPO 的 token 级别损失（停梯度）
        weighted_pg = tis_weight * base_pg
    else:
        weighted_pg = base_pg

    # ===== 掩码聚合 =====
    pg_loss = VF.masked_mean(weighted_pg, eos_mask)
    pg_clipfrac = VF.masked_mean((pg_losses2 > pg_losses).float(), eos_mask)
...
def kl_penalty(log_probs: torch.FloatTensor, ref_log_probs: torch.FloatTensor, kl_penalty: str, advantages=None) -> torch.Tensor:
    ...
    if kl_penalty == 'softplus':
        kl = ref_log_probs - log_probs
        kld = (kl.exp() - kl - 1).contiguous()
        norm_adv = torch.tanh(0.5 * advantages.detach())  # (-1, 1) 对称
        weights = 1.0 + norm_adv * 0.6  # [1-r, 1+r]
        return torch.clamp(kld * weights, min=-10, max=10)

    if kl_penalty == 'advantages':
        kl = ref_log_probs - log_probs
        kld = (kl.exp() - kl - 1).contiguous()

        # 只对 advantages > 0 的样本计算 KL，其余置 0
        # shape 4, 2048
