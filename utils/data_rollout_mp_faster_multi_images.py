...
class GropoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        step = self.data[idx]
        # 1) 获取所有图像路径
        image_paths = step["images"]

        # 2) 构建 prompt_text，其中插入 <image> 占位符
        #    如: "请分析以下图片。<image> 这是一张截图。<image> 请继续操作。"
        raw_prompt = GRPO_SYSTEM_PROMPT + step["problem"]
        prompt_parts = raw_prompt.split("<image>")
...
def worker(proc_id, num_procs, args):
    ...
    llm = LLM(model=args.model_path,
              limit_mm_per_prompt={"image":args.img_num, "video":0}, dtype="bfloat16")
