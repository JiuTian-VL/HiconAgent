...
from .configuration_qwen2_5_vl import XYQVLConfig, XYQVLVisionConfig
...
def pad_logits_with_prefix_and_postfix(
        logits: torch.Tensor,
        ...
        """
        Args:
            logits: [batch, seq_1, dim]
            prefix_pads: List[int], length=batch
            pad_value: float, value used for padding
            target_len: int, final sequence length (e.g. 4096)

        Returns:
            padded_logits: [batch, target_len, dim]
        """
        # print(logits.shape)
        ...
        for i in range(batch):
            prefix_len = prefix_pads[i]
            # postfix_len = target_len - prefix_len - seq_len

            # [prefix_len, dim]
            prefix_pad = torch.full((prefix_len, dim), pad_value, dtype=dtype, device=device)
            # print(prefix_pad)
            # [postfix_len, dim]
            # postfix_pad = torch.full((postfix_len, dim), pad_value, dtype=dtype, device=device)

            padded = torch.cat([prefix_pad, logits[i]], dim=0)  # [target_len, dim]
            if len(padded) > target_len:
                padded = padded[0:target_len]
            padded_logits.append(padded)

        # [batch, target_len, dim]
        return torch.stack(padded_logits, dim=0)


logger = logging.get_logger(__name__)

_CONFIG_FOR_DOC = "XYQVLConfig"
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    config_class = XYQVLVisionConfig
...
class Qwen2_5_VLRotaryEmbedding(nn.Module):
    def __init__(self, config: XYQVLConfig, device=None):
        super().__init__()
...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def __init__(self, config: XYQVLConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class XYQ_VLFlashAttention2(Qwen2_5_VLAttention):
    """
...
QWEN2_5_VL_ATTENTION_CLASSES = {
    ...
    "flash_attention_2": XYQ_VLFlashAttention2,
    "sdpa": Qwen2_5_VLSdpaAttention,
}


class Qwen2_5_VLDecoderLayer(nn.Module):
    def __init__(self, config: XYQVLConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    def __init__(self, config: XYQVLConfig):
    ...
    def Drop_hiddenstates(self, hidden, OAS_index, input_ids, position_embeddings):
        ...
        seq_length_before = [len(input_id) for input_id in input_ids]
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if not self.training:
                if i == self.drop_k and hidden_states.shape[1] != 1 and OAS_index is not None:
            ...
            # If training
                ...
                if i == self.drop_k and OAS_index is not None:
                    hidden_states, position_embeddings, padded_attention_mask = self.Drop_hiddenstates(hidden_states, OAS_index, input_ids_aux, position_embeddings)
                    causal_mask = self._update_causal_mask(
                        padded_attention_mask, hidden_states, cache_position, past_key_values, output_attentions
                    )
...
class XYQForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # processor = AutoProcessor.from_pretrained("/data1/Models/Qwen2.5-VL-3B-Instruct")
        ...
        # # 这里增加对于logits的处理
        # 按照answer的位置，把长度补充到和原来sequence一致
        # 首先按照OA对应减少的token补到前面，然后再补充padding到后面
        # print("after compression:", logits.shape)
        # print(OAS_index)
        if labels is None and OAS_index is not None:
            left_pad = []
            for i, cur_input_id in enumerate(input_ids):
                cur_index = OAS_index[i]
                cur_reduction_tokens = 0
                for j, index in enumerate(cur_index):
                    if index != []:
                            cur_reduction_tokens += (index[1] - index[0])
                left_pad.append(cur_reduction_tokens)

            logits = pad_logits_with_prefix_and_postfix(logits = logits, prefix_pads = left_pad, target_len=input_ids.shape[1])
            # print(input_ids.shape[1])
        
        # print("after padding:", logits.shape)
        # print(logits.shape)
...
AutoModelForVision2Seq.register(XYQVLConfig, XYQForConditionalGeneration)
print("xyqnb")
__all__ = ["XYQForConditionalGeneration", "Qwen2_5_VLModel", "Qwen2_5_VLPreTrainedModel"]
